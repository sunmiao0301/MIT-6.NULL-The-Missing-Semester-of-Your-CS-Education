...
哟，有意思，我们是**有**构建 `plot-data.png` 的规则的，但是这是一条模式规则。因为源文件`data.d at` 并不存在，因此 `make` 就会告诉您它不能构建 `plot-data.png`，让我们创建这些文件：
...
# vim Makefile
  ...
  4 paper.pdf: paper.tex plot-data.png
  3         pdflatex paper.tex
  2 
  1 plot-%.png: %.dat plot.py
5           ./plot.py -i $*.dat -0 $@
...
```bash
# have seen many people use :x to quit vim
# what's the difference between :wq and :x?

# :wq 和 :x 的区别
# 该:wq命令在 Vim 中用于写入和退出。缓冲区的内容被写入相关文件的磁盘，然后 Vim 会话终止。那么，这和:x命令有什么区别。Vim 帮助文件对:x命令给出了以下描述：

# 类似于“:wq”，但仅在进行了更改时才写入。

# 因此，:wq以任何一种方式将缓冲区写入磁盘，而:x如果缓冲区未更改则退出。无论哪种方式，生成的文件的内容都将相同。那么有什么区别呢？

# 修改时间。

# 如果您:x的缓冲区未更改，则修改时间将保持不变，因为文件未重新保存。:wq无论如何，该命令都会更改修改时间。

# 如果修改时间被任何东西使用，这很重要。例如，根据修改时间监视目录中已更改文件的后台进程，如果您:wq过于随意使用，将会得到一些误报。
```
